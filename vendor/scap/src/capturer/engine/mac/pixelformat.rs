...
pub unsafe fn create_bgr_frame(
    ...
    // The pixel buffer is BGRA with a per-row stride that can exceed
    // width*4. Copy row by row, dropping the alpha byte, so the result is
    // always exactly width*height*3 bytes of tightly-packed BGR regardless
    // of the plane's stride or plane height. (The previous crop helper
    // returned the padded buffer untouched when the sizes disagreed, which
    // sheared the picture.)
    let stride = image_buffer.plane_bytes_per_row(0);
    let plane_height = image_buffer.plane_height(0);
    let bytes = unsafe {
        std::slice::from_raw_parts(
            image_buffer.plane_base_address(0),
            stride * plane_height,
    ...
    for y in 0..height {
        ...
        for x in 0..width {
            ...
            data.extend_from_slice(&bytes[p..p + 3]);
        }
    }
