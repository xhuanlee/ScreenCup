...
/// Probes whether the process can actually capture frames, bypassing the
/// (sometimes unreliable) `CGPreflightScreenCaptureAccess` check. A
/// translocated or renamed copy of the app can keep reporting "not granted"
/// via preflight even after the user has granted permission, so the only
/// trustworthy test is to start a stream and see whether frames arrive.
pub fn probe_capture() -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // Everything is created inside the closure so it captures nothing and
        // is UnwindSafe. `Engine::new` unwraps the shareable-content call,
        // which fails (panics) precisely when permission is missing — the only
        // state this probe is ever invoked in — so a panic here simply means
        // "no permission".
        //
        // NOTE: deliberately NOT `get_all_targets()` either: it unwraps the
        // same failing call. The main display needs no permission to look up.
        let res = panic::catch_unwind(|| {
            use crate::targets::{get_main_display, Target};

            let target = Target::Display(get_main_display());
            let (tx, rx) = mpsc::channel();
            let options = Options {
                fps: 5,
                show_cursor: false,
                ...
                target: Some(target),
                crop_area: None,
                output_type: FrameType::BGR0,
                output_resolution: Resolution::Captured,
                excluded_targets: None,
                captures_audio: false,
                exclude_current_process_audio: false,
            };
            let mut engine = engine::Engine::new(&options, tx);
            engine.start();
            let got_frame = rx.recv_timeout(Duration::from_secs(3)).is_ok();
            engine.stop();
            got_frame
        });
        res.unwrap_or(false)
